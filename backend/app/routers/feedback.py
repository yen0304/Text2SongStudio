...
from app.models import Feedback, AudioSample
from app.schemas import FeedbackCreate, FeedbackResponse, FeedbackListResponse, FeedbackStatsResponse
...
async def submit_feedback(
    ...
    result = await db.execute(select(AudioSample).where(AudioSample.id == data.audio_id))
    ...
    if data.preferred_over:
        ...
        if not result.scalar_one_or_none():
            raise HTTPException(status_code=404, detail="Preferred-over audio sample not found")
...
async def get_feedback_stats(
    ...
    if audio_id:
        ...
        base_query = base_query.join(AudioSample).where(AudioSample.adapter_id == adapter_id)
    ...
    if audio_id:
        wins_result = await db.execute(
            select(func.count()).where(Feedback.audio_id == audio_id).where(Feedback.preferred_over.isnot(None))
        )
